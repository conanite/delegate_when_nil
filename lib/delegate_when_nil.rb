...
module DelegateWhenNil
  ...
  # same as #delegate_when_nil except also delegates when blank (eg "" or [] or {})
  def delegate_when_blank *names
    opts     = names.pop
    raise "please provide a hash with keys :to and :prefix as last argument" unless opts.is_a?(Hash) && opts.key?(:to) && opts.key?(:prefix)
    ...
    names.each { |name|
      myname = [prefix, name].compact.join('_')
      fbname = stop ? name : myname
      class_eval "def #{myname}(*args) ; self.#{name}(*args).nb || (#{fallback} && #{fallback}.#{fbname}(*args)) ; end"
    }
  end
end
